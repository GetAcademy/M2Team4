...
function createWeeksHtml() {
    ...
    for (let weekNo = 1; weekNo < 9; weekNo++) {
        let style = model.inputs.logPage.selectedWeek == weekNo ? 'background-color: lightgreen' : '';
        html += /*HTML*/`
            <button 
...
function createDaysHtml() {
    ...
    for (let dayNo = 1; dayNo < 6; dayNo++) {
        ...
        let style = '';
        if (model.inputs.logPage.selectedDay == dayNo) style = 'background-color: lightgreen';
        html += /*HTML*/`
            <button 
...
function createQuestionsHtml() {
    let html = '';
    for(let questionIndex = 0; questionIndex < model.mainQuestions.length; questionIndex++){
